...
| GEOGLOWS v2 return periods (the model behind INAMHI's portal, `inamhi.geoglows.org`) | 2- to 100-year flood flows at the river each dam stands on, from a Gumbel fit on the 1940 → retrospective simulation; read from the public `geoglows-v2` bucket by `npm run geoglows`. **CC BY-NC-SA 4.0** | reference, store revision 2026-06-10 |
...
npm run geoglows                           # GEOGLOWS return periods at each dam's river, beside CELEC's own fit
...
- **`inflow.return_periods`** in `latest.json` places each reservoir's inflow against two sets of
  2- to 100-year flood flows kept side by side: `geoglows` (GEOGLOWS v2, the model INAMHI's portal
  shows, at the river matched to the dam's catchment) and `measured` (the same Gumbel fit on
  CELEC's own daily record, from five complete years). They agree at Mazar and Coca Codo Sinclair
  and differ by 26–133% at four other dams — `data/reports/return-periods.md` has the comparison,
  and PLAN.md §2.8 and Phase 7 the method.
