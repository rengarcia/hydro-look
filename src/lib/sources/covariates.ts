...
export function loadBasins(): Basin[] {
  ...
  const basins = z.array(basinSchema).min(1).parse(parseCsv(readFileSync(join(DATA_REFERENCE, "basins.csv"), "utf8")));
  if (new Set(basins.map((b) => b.basin)).size !== basins.length) throw new Error("duplicate basin id");
  return basins;
}

export class Covariates {
  constructor(private readonly http: Pick<HttpClient, "fetch">, private readonly archive: RawArchive) {}
  ...
  async weather(batch: IngestBatch, basin: Basin, kind: "era5" | "forecast", from: string, to: string): Promise<void> {
    ...
    try {
      const result = await this.http.fetch({
        ...
        params: {
          latitude: String(basin.latitude), longitude: String(basin.longitude),
          daily: "precipitation_sum,temperature_2m_mean", timezone: "America/Guayaquil",
      ...
      batch.weather.push(...validateRows(WEATHER_DAILY, rows.map((row) => ({
        ...row, basin: basin.basin, latitude: basin.latitude, longitude: basin.longitude, kind,
        issued_at: kind === "forecast" ? result.fetchedAt : "",
        source: `open_meteo:${kind}`, fetched_at: result.fetchedAt, raw_ref: rawRef,
      }))));
    } catch (error) {
  ...
  async oni(batch: IngestBatch): Promise<void> {
    try {
      ...
      const rawRef = this.archive.add("noaa", "oni", null, {
        key: `oni:${result.fetchedAt}`, url: result.url, method: result.method,
        status: result.status, fetched_at: result.fetchedAt, body: result.body,
      });
      if (result.status !== 200) throw new Error(`HTTP ${result.status} (archived at ${rawRef})`);
      batch.enso.push(...validateRows(ENSO_MONTHLY, parseOni(result.body).map((row) => ({
        ...row, source: "noaa_psl:oni", fetched_at: result.fetchedAt, raw_ref: rawRef,
      }))));
    } catch (error) {
...
export async function ingestCovariates(
  source: Covariates, store: CuratedStore, batch: IngestBatch, options: Options, today = todayEc(),
): Promise<void> {
  ...
  const from = z.string().date().parse(options.from ?? addDays(available, -29));
  const requestedTo = z.string().date().parse(options.to ?? available);
  ...
  for (const basin of basins) {
    if (basin.coordinate_status === "provisional") batch.notes.push(`${basin.basin}: provisional sampling point, not a verified basin centroid`);
    ...
    for (let year = yearOf(from); year <= yearOf(to); year++) {
      ...
      if (options.from) {
        const complete = new Set(store.read(join(DATA_CURATED, "weather_daily", `${year}.csv`))
          .filter((r) => r["basin"] === basin.basin && r["kind"] === "era5" &&
            Number(r["latitude"]) === basin.latitude && Number(r["longitude"]) === basin.longitude &&
            Number.isFinite(Number(r["precip_mm"])) && Number.isFinite(Number(r["temp_mean_c"])) &&
            r["precip_mm"] !== "" && r["temp_mean_c"] !== "")
          .map((r) => r["date"]));
