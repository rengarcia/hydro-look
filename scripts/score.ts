 ...
 * patches only the `scorecard` key of each document and leaves every other byte as it was.
 *
...
function main(): void {
  ...
  const patches: [string, Record<string, unknown>][] = [
    [join(apiDir, "forecast.json"), scorecardBlock(level, generatedAt)],
    [join(apiDir, "adequacy.json"), scorecardBlock(adequacy, generatedAt)],
  ];
  ...
  for (const [path, block] of patches) {
    if (!existsSync(path)) {
