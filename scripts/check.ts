...
const FRESHNESS_LIMIT_DAYS: Record<string, number> = {
  ...
  "Open-Meteo ERA5": 10,
  "NOAA ONI": 110,
  // XM settles the cross-border flows about three days after the fact and storage after one
  // (measured on the recon capture of 2026-09-22); the XM step runs once a day, so a limit a few
  // days past each lag trips on a stopped feed without tripping on a slow week.
  "XM exchanges with Colombia": 7,
  "XM Colombian storage": 4,
  // GEOGLOWS issues a forecast every day at 00 UTC and the daily model step fetches it; a failed
  // fetch only leaves the inflow ensemble with two members that day, so the limit allows two
  // missed days before a stopped fetch is reported.
  "GEOGLOWS river forecast": 3,
  // Regenerated by the same daily job that ingests, from data already on disk. It has no
...
function main(): void {
  ...
  const xmExchange = readTable(XM_EXCHANGE_DAILY.name);
  const xmSystem = readTable(XM_SYSTEM_DAILY.name);
  ...
  const rules: FreshnessRule[] = [
    ...
    ["GEOGLOWS river forecast", latestWhere(geoglowsForecasts.rows, "issued", () => true)],
